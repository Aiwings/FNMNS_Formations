...
function modif_formation()
{
	if ( isset( $_POST["submit"]) && isset( $_POST["id"]) )
	{
		...
		{
			$result_update = $wpdb->update( 'formation', 					
				array( 
		...
		{		
			$sql_select = 'SELECT * FROM `discipline` WHERE `discipline` = "'.$_POST["discipline"].'";';
			...
			if ( sizeof($reponse_select) != 1) 
			{
					...
					$results_disc = $wpdb->insert('discipline',array(
						"discipline"=>$_POST["discipline"]
					));
		
				if($results_disc == 1)
				{
					$sql_discipline = 'SELECT id FROM `discipline` WHERE `discipline` = "'.$_POST["discipline"].'" ;';	
